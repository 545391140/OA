...
// ============================================
const OCR_PROMPT = `请识别这张发票图片中的所有文字内容，以 markdown 格式输出。

**重要要求：**
1. **完整提取**：必须识别发票上的所有文字，包括但不限于：
   - 发票号码、发票代码、开票日期
   - 销售方（开票方）的完整信息：名称、税号、地址、电话等
   - 购买方（买方）的完整信息：名称、税号、地址、电话等
   - 项目明细表格中的所有数据（项目名称、规格型号、单位、数量、单价、金额、税率、税额）
   - 金额信息（合计金额、税额、价税合计）
   - 开票人、备注、印章文字等所有可见信息

2. **顺序识别**：按照从左到右、从上到下的顺序识别，确保不遗漏任何信息

3. **表格格式**：如果发票中有表格，请以 markdown 表格格式输出，保留所有列和行

4. **完整输出**：不要省略任何信息，即使是小字、备注、边角信息也要提取

请以 markdown 格式输出所有识别到的文字内容。`;
