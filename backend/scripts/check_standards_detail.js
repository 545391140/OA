...
const connectDB = async () => {
  try {
    const conn = await mongoose.connect('mongodb+srv://liuzhijiansun:BE12mjA8imCd4vBp@cluster0.tzxphum.mongodb.net/travel-expense-system?retryWrites=true&w=majority');
...
const checkStandards = async () => {
  ...
  try {
    ...
    for (const std of standards) {
      ...
      // 尝试查询关联的费用标准项
      const TransportStandard = require('../models/TravelTransportStandard');
      const AccommodationStandard = require('../models/TravelAccommodationStandard');
      const AllowanceStandard = require('../models/TravelAllowanceStandard');
      const OtherStandard = require('../models/TravelOtherExpenseStandard');
