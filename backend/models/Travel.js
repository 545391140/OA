    ...
    setImmediate(async () => {
      try {
        ...
        if (travel && travel.status === 'completed' && 
          ...
          logger.info(`Auto-generated expenses for travel ${travel.travelNumber || travel._id}`);
        }
      } catch (error) {
        logger.error('Failed to auto-generate expenses:', error);
        ...
        try {
          ...
          logger.error('Failed to update travel error status:', updateError);
        }
      } finally {
        // 清除标记
        if (doc._autoGeneratingExpenses) {
